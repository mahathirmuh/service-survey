// Run this script to add external_assetprotection columns to the survey_responses table
...
// SQL to add external_assetprotection columns
const addColumnsSQL = `
...
async function executeSQLViaRPC() {
  try {
    console.log('🔍 Attempting to add external_assetprotection columns via RPC...');
...
async function verifyColumns() {
  try {
    console.log('🔍 Verifying external_assetprotection columns...');
    
    // Try to select from the table with the new columns
    const { data, error } = await supabase
      .from('survey_responses')
      .select('external_assetprotection_feedback')
      .limit(1);
    ...
    console.log('✅ External_assetprotection columns verified');
    return true;
  } catch (err) {
...
// Main function
async function main() {
  console.log('🚀 Starting external_assetprotection columns addition...');
  ...
  if (columnsExist) {
    console.log('🎉 External_assetprotection columns added successfully!');
    
    // Run the import script
    await runImportScript();
    
    console.log('\n📋 Migration process summary:');
    console.log('1. ✅ Added external_assetprotection columns to survey_responses table');
    ...
    console.log('❌ Failed to add external_assetprotection columns');
